...
func handlerUnfollow(s *state, cmd command, user database.User) error {
	...
	err := s.db.DeleteFeedFollowss(context.Background(), database.DeleteFeedFollowssParams{
		Name: user.Name, Url: cmd.Args[0],
	})
	if err != nil {
		return fmt.Errorf("unable to delete feeds for user %s and url %s: %v", user.Name, cmd.Args[0], err)
	}
	return nil
}
